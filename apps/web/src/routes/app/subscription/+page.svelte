...
<div class="space-y-6 py-8">
	{#if $subscriptionStore.sub && $subscriptionStore.sub.status === 'active'}
		<div class="grid gap-4 lg:grid-cols-[auto_1fr] lg:gap-10">
			<div class="grid justify-items-center">
				<h2 class="text-3xl text-accent">
					{$subscriptionStore.tier?.name} / {$subscriptionStore.sub.plan.interval}ly
				</h2>
				<img src={$subscriptionStore.tier?.image} alt="" class="max-w-sm" />
			</div>
			...
			<div class="space-y-6">
				{#if $subscriptionStore.paymentMethod}
					<Card.Root class="border-2 border-accent-foreground">
						...
						<Card.Content>
							<p class="font-bold">
								<span class="text-xl uppercase">{$subscriptionStore.paymentMethod.card?.brand}</span
								>

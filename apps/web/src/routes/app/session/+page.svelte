<script lang="ts">
	...
	async function startNewSession(profileId: string, crewId: string, title: string) {
        const session: models.Session | false = await api.startSession(profileId as UUID, crewId as UUID, title);
...
{#if crews}
	...
	>
		...
		{#if crews}
			...
			<Button on:click={() => startNewSession(profileId, crews[0].id, 'New Session')}>Run Your Crew!</Button>
		{:else}
			<p>Loading...</p>
		{/if}
	</div>

...
class RetConversableAgent(LLMAgent):
    ...
    def __init__(
        ...
        if llm_config is False:
            ...
            if "model" not in self.llm_config and (
                ...
                or any(
                    not config.get("model") for config in self.llm_config["config_list"]
                )
            ):
                raise ValueError(
        ...
        self.register_reply([Agent, None], RetConversableAgent.generate_oai_reply)
        self.register_reply(
            [Agent, None],
            RetConversableAgent.a_generate_oai_reply,
        ...
        if code_execution_config is not False:
            ...
            if self._code_execution_config.get("executor") is not None:
                ...
                self.register_reply(
                    [Agent, None],
                    RetConversableAgent._generate_code_execution_reply_using_executor,
                )
            else:
                ...
                self.register_reply(
                    [Agent, None], RetConversableAgent.generate_code_execution_reply
                )
        else:
        ...
        self.register_reply(
            [Agent, None], RetConversableAgent.generate_tool_calls_reply
        )
        self.register_reply(
            [Agent, None],
            RetConversableAgent.a_generate_tool_calls_reply,
            ignore_async_in_sync_chat=True,
        )
        self.register_reply(
            [Agent, None], RetConversableAgent.generate_function_call_reply
        )
        self.register_reply(
            [Agent, None],
            RetConversableAgent.a_generate_function_call_reply,
            ignore_async_in_sync_chat=True,
        )
        self.register_reply(
            [Agent, None], RetConversableAgent.check_termination_and_human_reply
        )
        self.register_reply(
            [Agent, None],
            RetConversableAgent.a_check_termination_and_human_reply,
            ignore_async_in_sync_chat=True,
        )
    ...
    def register_reply(
        ...
        *,
        ...
        chats (initiated with `RetConversableAgent.a_initiate_chat`). If an `async` reply function is registered
    ...
    def _raise_exception_on_async_reply_functions(self) -> None:
        ...
        if async_reply_functions != []:
            msg = (
                "Async reply functions can only be used with RetConversableAgent.a_initiate_chat(). The following async reply functions are found: "
    ...
    def initiate_chat(
        ...
        **context,
        ...
        summary = self._summarize_chat(
            context.get("summary_method", RetConversableAgent.DEFAULT_summary_method),
            recipient,
    ...
    async def a_initiate_chat(
        ...
        **context,
        ...
        for agent in [self, recipient]:
            ...
            agent.client_cache = cache
        await self.a_send(
            await self.a_generate_init_message(**context), recipient, silent=silent
        )
        summary = self._summarize_chat(
            context.get("summary_method", RetConversableAgent.DEFAULT_summary_method),
            recipient,
    ...
    def _summarize_chat(
        ...
        if summary_method == "reflection_with_llm":
            prompt = (
                RetConversableAgent.DEFAULT_summary_prompt if prompt is None else prompt
            )
            if not isinstance(prompt, str):
                raise ValueError("The summary_prompt must be a string.")
            msg_list = agent.chat_messages_for_summary(self)
            try:
                summary = self._reflection_with_llm(
                    prompt, msg_list, llm_agent=agent, cache=cache
                )
            except BadRequestError as e:
                warnings.warn(
                    f"Cannot extract summary using reflection_with_llm: {e}",
                    UserWarning,
                )
            ...
            try:
                ...
                warnings.warn(
                    f"Cannot extract summary using last_msg: {e}", UserWarning
                )
    ...
    def get_chat_results(
        self, chat_index: Optional[int] = None
    ...
    async def a_generate_oai_reply(
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None,
            functools.partial(
                self.generate_oai_reply, messages=messages, sender=sender, config=config
            ),
        )
    ...
    def _match_trigger(
        self, trigger: Union[None, str, type, Agent, Callable, List], sender: Agent
    ) -> bool:
        ...
        if trigger is None:
            ...
            assert rst in [
                True,
                False,
            ], f"trigger {trigger} must return a boolean value."
            return rst
        ...
        if is_remove:
            if "functions" not in self.llm_config.keys():
                ...
                self.llm_config["functions"] = [
                    func
                    for func in self.llm_config["functions"]
    ...
    def register_hook(self, hookable_method: Callable, hook: Callable):
        ...
        Args:
            hookable_method: A hookable method implemented by RetConversableAgent.
            hook: A method implemented by a subclass of AgentCapability.
        """
        assert (
            hookable_method in self.hook_lists
        ), f"{hookable_method} is not a hookable method."
...
def register_function(
    f: Callable[..., Any],
    *,
    caller: RetConversableAgent,
    executor: RetConversableAgent,
    name: Optional[str] = None,
    description: str,
) -> None:
    ...
    Args:
        ...
        caller: the agent calling the function, typically an instance of RetConversableAgent.
